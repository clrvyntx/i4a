...
static int listen_sock = -1;  // Declare listening socket globally
static bool server_is_up = false;  // Flag to control the server loop
...
static void tcp_server_task(void *pvParameters) {
  ...
  CLEAN_UP:
  if (listen_sock >= 0) {
    close(listen_sock);  // Close listening socket if it's still open
  }
...
void server_close() {
  if (server_is_up) {
    server_is_up = false;  // Stop accepting new connections

    // Ensure cleanup of the client socket if connected
    if (client_sock >= 0) {
      ESP_LOGI(LOGGING_TAG, "Closing client connection...");
      shutdown(client_sock, 0);
      close(client_sock);
      client_sock = -1;
    }

    // If the server is listening, stop listening and close the server socket
    if (listen_sock >= 0) {
      ESP_LOGI(LOGGING_TAG, "Closing server socket...");
      close(listen_sock);
      listen_sock = -1;
    }

    ESP_LOGI(LOGGING_TAG, "Server has been closed.");
  } else {
