...
esp_err_t node_start_event_tasks(void) {
    ...
    res = xTaskCreatePinnedToCore(peer_connected_task, "peer_conn_task", 4096, NULL, (tskIDLE_PRIORITY + 2), NULL, 0);
    if (res != pdPASS) {
        ESP_LOGE(TAG, "Failed to create peer_connected_task");
        return ESP_FAIL;
    }

    res = xTaskCreatePinnedToCore(peer_lost_task, "peer_lost_task", 4096, NULL, (tskIDLE_PRIORITY + 2), NULL, 0);
    if (res != pdPASS) {
