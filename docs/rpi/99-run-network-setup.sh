...
#  - Adds a route to the 10.0.0.0/8 network via the access point at 10.160.0.1
...
if [ "$IFACE" == "wlan0" ] && [ "$STATUS" == "up" ]; then
    ...
    ip route del default via 10.160.0.2 dev wlan0 2>/dev/null

    # Route all 10.0.0.0/8 traffic to the AP bridge at 10.160.0.1
    ip route add 10.0.0.0/8 via 10.160.0.1 dev wlan0
